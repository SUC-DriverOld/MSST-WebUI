...
class Separator:
    ...
    def separate(self, folder_path):
        """
        Separates all audio files in the specified folder into different stems (e.g., vocals, instruments) using the loaded model.

        This method takes the path to a folder containing audio files, processes each file through the loaded separation model,
        and returns the paths to the output files containing the separated audio stems for all files in the folder.

        Parameters:
        - folder_path (str): The path to the folder containing the audio files to be separated.

        Returns:
        - output_files (list of str): A list containing the paths to the separated audio stem files for all audio files in the folder.
        """
        # 检查输入的是路径还是文件
        ...
        for filename in os.listdir(folder_path):
            ...
            # Check if the file is an audio file (you can customize this check as needed)
            # 说实话我不知道uvr5支持多少音频格式，所以这边就写了一些常见的格式...
            if not filename.lower().endswith(('.mp3', '.wav', '.flac', '.aac', '.m4a', '.ogg', '.wma')): 
                self.logger.warning(f"Skipping non-audio file: {file_path}")
                continue

            # Perform separation on the audio file
            file_output_files = self.separate_onefile(file_path)

            # Add the output files to the main list
