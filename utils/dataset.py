...
def get_track_set_length(params):
	...
	if lengths_arr.min() != lengths_arr.max():
		logger.warning("lengths of stems are different for path: {}. ({} != {})".format(path, lengths_arr.min(), lengths_arr.max()))
		...
		if self.dataset_type in [1, 4]:
			...
			if read_metadata_procs <= 1:
				...
				with tqdm(total=len(track_paths)) as pbar:
					track_iter = p.imap(get_track_set_length, zip(track_paths, itertools.repeat(self.instruments), itertools.repeat(self.file_types)))
			...
			if self.aug:
				if "mixup" in self.config["augmentations"]:
					if self.config["augmentations"].mixup:
						...
						loud_values = np.random.uniform(low=self.config.augmentations.loudness_min, high=self.config.augmentations.loudness_max, size=(len(mixup),))
		...
		if "pitch_shift" in augs:
			if augs["pitch_shift"] > 0:
				if random.uniform(0, 1) < augs["pitch_shift"]:
					apply_aug = AU.PitchShift(min_semitones=augs["pitch_shift_min_semitones"], max_semitones=augs["pitch_shift_max_semitones"], p=1.0)
		...
		if "seven_band_parametric_eq" in augs:
			if augs["seven_band_parametric_eq"] > 0:
				if random.uniform(0, 1) < augs["seven_band_parametric_eq"]:
					apply_aug = AU.SevenBandParametricEQ(min_gain_db=augs["seven_band_parametric_eq_min_gain_db"], max_gain_db=augs["seven_band_parametric_eq_max_gain_db"], p=1.0)
					source = apply_aug(samples=source, sample_rate=44100)
		...
		if "tanh_distortion" in augs:
			if augs["tanh_distortion"] > 0:
				if random.uniform(0, 1) < augs["tanh_distortion"]:
					apply_aug = AU.TanhDistortion(min_distortion=augs["tanh_distortion_min"], max_distortion=augs["tanh_distortion_max"], p=1.0)
					source = apply_aug(samples=source, sample_rate=44100)
		...
		if "mp3_compression" in augs:
			if augs["mp3_compression"] > 0:
				if random.uniform(0, 1) < augs["mp3_compression"]:
					apply_aug = AU.Mp3Compression(min_bitrate=augs["mp3_compression_min_bitrate"], max_bitrate=augs["mp3_compression_max_bitrate"], backend=augs["mp3_compression_backend"], p=1.0)
					source = apply_aug(samples=source, sample_rate=44100)
		...
		if "gaussian_noise" in augs:
			if augs["gaussian_noise"] > 0:
				if random.uniform(0, 1) < augs["gaussian_noise"]:
					apply_aug = AU.AddGaussianNoise(min_amplitude=augs["gaussian_noise_min_amplitude"], max_amplitude=augs["gaussian_noise_max_amplitude"], p=1.0)
					source = apply_aug(samples=source, sample_rate=44100)
		...
		if "time_stretch" in augs:
			if augs["time_stretch"] > 0:
				if random.uniform(0, 1) < augs["time_stretch"]:
					apply_aug = AU.TimeStretch(min_rate=augs["time_stretch_min_rate"], max_rate=augs["time_stretch_max_rate"], leave_length_unchanged=True, p=1.0)
					source = apply_aug(samples=source, sample_rate=44100)
		...
		if "pedalboard_reverb" in augs:
			if augs["pedalboard_reverb"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_reverb"]:
					room_size = random.uniform(augs["pedalboard_reverb_room_size_min"], augs["pedalboard_reverb_room_size_max"])
					damping = random.uniform(augs["pedalboard_reverb_damping_min"], augs["pedalboard_reverb_damping_max"])
					wet_level = random.uniform(augs["pedalboard_reverb_wet_level_min"], augs["pedalboard_reverb_wet_level_max"])
					dry_level = random.uniform(augs["pedalboard_reverb_dry_level_min"], augs["pedalboard_reverb_dry_level_max"])
					width = random.uniform(augs["pedalboard_reverb_width_min"], augs["pedalboard_reverb_width_max"])
					board = PB.Pedalboard(
						[
							PB.Reverb(
		...
		if "pedalboard_chorus" in augs:
			if augs["pedalboard_chorus"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_chorus"]:
					rate_hz = random.uniform(augs["pedalboard_chorus_rate_hz_min"], augs["pedalboard_chorus_rate_hz_max"])
					depth = random.uniform(augs["pedalboard_chorus_depth_min"], augs["pedalboard_chorus_depth_max"])
					centre_delay_ms = random.uniform(augs["pedalboard_chorus_centre_delay_ms_min"], augs["pedalboard_chorus_centre_delay_ms_max"])
					feedback = random.uniform(augs["pedalboard_chorus_feedback_min"], augs["pedalboard_chorus_feedback_max"])
					mix = random.uniform(augs["pedalboard_chorus_mix_min"], augs["pedalboard_chorus_mix_max"])
					board = PB.Pedalboard([PB.Chorus(rate_hz=rate_hz, depth=depth, centre_delay_ms=centre_delay_ms, feedback=feedback, mix=mix)])
					source = board(source, 44100)
		...
		if "pedalboard_phazer" in augs:
			if augs["pedalboard_phazer"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_phazer"]:
					rate_hz = random.uniform(augs["pedalboard_phazer_rate_hz_min"], augs["pedalboard_phazer_rate_hz_max"])
					depth = random.uniform(augs["pedalboard_phazer_depth_min"], augs["pedalboard_phazer_depth_max"])
					centre_frequency_hz = random.uniform(augs["pedalboard_phazer_centre_frequency_hz_min"], augs["pedalboard_phazer_centre_frequency_hz_max"])
					feedback = random.uniform(augs["pedalboard_phazer_feedback_min"], augs["pedalboard_phazer_feedback_max"])
					mix = random.uniform(augs["pedalboard_phazer_mix_min"], augs["pedalboard_phazer_mix_max"])
					board = PB.Pedalboard([PB.Phaser(rate_hz=rate_hz, depth=depth, centre_frequency_hz=centre_frequency_hz, feedback=feedback, mix=mix)])
					source = board(source, 44100)
		...
		if "pedalboard_distortion" in augs:
			if augs["pedalboard_distortion"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_distortion"]:
					drive_db = random.uniform(augs["pedalboard_distortion_drive_db_min"], augs["pedalboard_distortion_drive_db_max"])
		...
		if "pedalboard_pitch_shift" in augs:
			if augs["pedalboard_pitch_shift"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_pitch_shift"]:
					semitones = random.uniform(augs["pedalboard_pitch_shift_semitones_min"], augs["pedalboard_pitch_shift_semitones_max"])
		...
		if "pedalboard_resample" in augs:
			if augs["pedalboard_resample"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_resample"]:
					target_sample_rate = random.uniform(augs["pedalboard_resample_target_sample_rate_min"], augs["pedalboard_resample_target_sample_rate_max"])
					board = PB.Pedalboard([PB.Resample(target_sample_rate=target_sample_rate)])
					source = board(source, 44100)
		...
		if "pedalboard_mp3_compressor" in augs:
			if augs["pedalboard_mp3_compressor"] > 0:
				if random.uniform(0, 1) < augs["pedalboard_mp3_compressor"]:
					vbr_quality = random.uniform(augs["pedalboard_mp3_compressor_pedalboard_mp3_compressor_min"], augs["pedalboard_mp3_compressor_pedalboard_mp3_compressor_max"])
		...
		if self.aug:
			if "loudness" in self.config["augmentations"]:
				if self.config["augmentations"]["loudness"]:
					loud_values = np.random.uniform(low=self.config["augmentations"]["loudness_min"], high=self.config["augmentations"]["loudness_max"], size=(len(res),))
					loud_values = torch.tensor(loud_values, dtype=torch.float32)
