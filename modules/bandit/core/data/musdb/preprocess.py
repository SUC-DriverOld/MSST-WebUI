...
class SourceActivityDetector(nn.Module):
	...
	def forward(self, data: DataDict) -> None:
		...
		n_segments = int(np.ceil((n_samples - self.segment_length) / self.hop_length)) + 1
		...
		for i in range(n_segments):
			...
			if end - start < self.segment_length:
				xseg = F.pad(xseg, pad=(0, self.segment_length - (end - start)), value=torch.nan)
		...
		if self.analysis_stem != "none":
			...
			energy_threshold = torch.nanquantile(chunk_energies, q=self.energy_threshold_quantile)
			...
			n_chunks_above_threshold = torch.mean(chunks_above_threshold.to(torch.float), dim=-1)

			segment_above_threshold = n_chunks_above_threshold > self.salient_proportion_threshold
		...
		for i in range(n_segments):
			...
			outpath = os.path.join(self.output_path, self.analysis_stem, f"{data['track']} - {self.analysis_stem}{i:03d}")
			...
			for stem in data["audio"]:
				if stem == self.analysis_stem:
					...
					if end - start < self.segment_length:
						segment = F.pad(segment, (0, self.segment_length - (end - start)))
...
def preprocess(
	...
	for split in ["train", "val", "test"]:
		ds = MUSDB18FullTrackDataset(data_root="/data/MUSDB18/HQ/canonical", split=split)
