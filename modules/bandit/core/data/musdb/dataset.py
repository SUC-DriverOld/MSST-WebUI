...
class MUSDB18BaseDataset(BaseSourceSeparationDataset, ABC):
	...
	def __init__(self, split: str, stems: List[str], files: List[str], data_path: str, fs: int = 44100, npy_memmap=False) -> None:
		super().__init__(split=split, stems=stems, files=files, data_path=data_path, fs=fs, npy_memmap=npy_memmap, recompute_mixture=False)
...
class MUSDB18FullTrackDataset(MUSDB18BaseDataset):
	...
	def __init__(self, data_root: str, split: str, stems: Optional[List[str]] = None) -> None:
		if stems is None:
...
class MUSDB18SadDataset(MUSDB18BaseDataset):
	def __init__(self, data_root: str, split: str, target_stem: str, stems: Optional[List[str]] = None, target_length: Optional[int] = None, npy_memmap=False) -> None:
		if stems is None:
			stems = self.ALLOWED_STEMS
		...
		super().__init__(data_path=data_path, split=split, stems=stems, files=files, npy_memmap=npy_memmap)
		self.n_segments = len(files)
		self.target_stem = target_stem
		self.target_length = target_length if target_length is not None else self.n_segments
	...
	def __getitem__(self, index: int) -> DataDict:
		...
		for stem in self.stems_no_mixture:
			...
			if self.chunk_size_sample < audio[stem].shape[-1]:
				chunk_start = np.random.randint(audio[stem].shape[-1] - self.chunk_size_sample)
			else:
				chunk_start = 0
			...
			# print(linear_scale)
			audio[stem][..., chunk_start : chunk_start + self.chunk_size_sample] = linear_scale * audio[stem][..., chunk_start : chunk_start + self.chunk_size_sample]

		audio["mixture"] = self.compute_mixture(audio)

		if self.rescale:
			max_abs_val = max([torch.max(torch.abs(audio[stem])) for stem in self.stems])  # type: ignore[type-var]
			if max_abs_val > 1:
