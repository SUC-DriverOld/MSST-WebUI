...
class DivideAndRemasterBaseDataset(BaseSourceSeparationDataset, ABC):
	...
	def get_stem(self, *, stem: str, identifier: Dict[str, Any]) -> torch.Tensor:
		...
		if self.npy_memmap:
			audio = np.load(os.path.join(path, f"{self.STEM_NAME_MAP[stem]}.npy"), mmap_mode="r")
		else:
...
class DivideAndRemasterDataset(DivideAndRemasterBaseDataset):
	def __init__(self, data_root: str, split: str, stems: Optional[List[str]] = None, fs: int = 44100, npy_memmap: bool = True) -> None:
		...
		files = [f for f in files if (not f.startswith(".")) and os.path.isdir(os.path.join(data_path, f))]
		...
		super().__init__(data_path=data_path, split=split, stems=stems, files=files, fs=fs, npy_memmap=npy_memmap)
...
class DivideAndRemasterRandomChunkDataset(DivideAndRemasterBaseDataset):
	...
	def get_stem(self, *, stem: str, identifier: Dict[str, Any], chunk_here: bool = False) -> torch.Tensor:
...
class DivideAndRemasterDeterministicChunkDataset(DivideAndRemasterBaseDataset):
	def __init__(self, data_root: str, split: str, chunk_size_second: float, hop_size_second: float, stems: Optional[List[str]] = None, fs: int = 44100, npy_memmap: bool = True) -> None:
		...
		self.n_chunks_per_track = int((self.FULL_TRACK_LENGTH_SECOND - chunk_size_second) / hop_size_second)
...
class DivideAndRemasterRandomChunkDatasetWithSpeechReverb(DivideAndRemasterRandomChunkDataset):
	def __init__(self, data_root: str, split: str, target_length: int, chunk_size_second: float, stems: Optional[List[str]] = None, fs: int = 44100, npy_memmap: bool = True) -> None:
		...
		super().__init__(data_root=data_root, split=split, target_length=target_length, chunk_size_second=chunk_size_second, stems=stems_no_mixture, fs=fs, npy_memmap=npy_memmap)
	...
	def __getitem__(self, index: int) -> DataDict:
		...
		speech = pb.Reverb(room_size=np.random.rand(), damping=np.random.rand(), wet_level=wet_level, dry_level=(1 - wet_level), width=np.random.rand()).process(dry, self.fs, buffer_size=8192 * 4)[
			..., :n_samples
		]

		data_["audio"]["speech"] = speech

		data_["audio"]["mixture"] = sum([data_["audio"][s] for s in self.stems_no_mixture])
...
if __name__ == "__main__":
	...
	for split_ in ["train", "val", "test"]:
		ds = DivideAndRemasterRandomChunkDatasetWithSpeechReverb(data_root="$DATA_ROOT/DnR/v2np", split=split_, target_length=100, chunk_size_second=6.0)
