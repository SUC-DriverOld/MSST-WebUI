...
class DPRNN(nn.Module):
	...
	def __init__(self, feat_dim: int = 64, hidden_dim: int = 128, num_blocks: int = 6, rnn_type: str = "LSTM", d_model: int = 256, chunk_size: int = 100, chunk_stride: int = 50) -> None:
		...
		self.conv = nn.Sequential(nn.Conv2d(feat_dim, d_model, 1), nn.PReLU())
...
def squim_objective_model(
	feat_dim: int, win_len: int, d_model: int, nhead: int, hidden_dim: int, num_blocks: int, rnn_type: str, chunk_size: int, chunk_stride: Optional[int] = None
) -> SquimObjective:
...
SQUIM_OBJECTIVE = SquimObjectiveBundle("squim_objective_dns2020.pth", _sample_rate=16000)
