...
@torch.jit.script
def merge(combined: torch.Tensor, original_batch_size: int, n_channel: int, n_chunks: int, chunk_size: int):
	combined = torch.reshape(combined, (original_batch_size, n_chunks, n_channel, chunk_size))
	combined = torch.permute(combined, (0, 2, 3, 1)).reshape(original_batch_size * n_channel, chunk_size, n_chunks)

	return combined


@torch.jit.script
def unfold(padded_audio: torch.Tensor, original_batch_size: int, n_channel: int, chunk_size: int, hop_size: int) -> torch.Tensor:
	unfolded_input = F.unfold(padded_audio[:, :, None, :], kernel_size=(1, chunk_size), stride=(1, hop_size))

	_, _, n_chunks = unfolded_input.shape
	unfolded_input = unfolded_input.view(original_batch_size, n_channel, chunk_size, n_chunks)
	unfolded_input = torch.permute(unfolded_input, (0, 3, 1, 2)).reshape(original_batch_size * n_chunks, n_channel, chunk_size)
...
def merge_chunks_all(
	...
	combined = F.fold(combined.to(torch.float32), output_size=(1, n_padded_samples), kernel_size=(1, chunk_size), stride=(1, hop_size))
...
def merge_chunks_edge(
	...
	combined = F.fold(combined, output_size=(1, n_padded_samples), kernel_size=(1, chunk_size), stride=(1, hop_size))
...
class BaseFader(nn.Module):
	def __init__(self, chunk_size_second: float, hop_size_second: float, fs: int, fade_edge_frames: bool, batch_size: int) -> None:
		super().__init__()
	...
	def forward(self, audio: torch.Tensor, model_fn: Callable[[torch.Tensor], Dict[str, torch.Tensor]]):
		...
		if n_channel > 1:
			padded_audio = padded_audio.view(original_batch_size * n_channel, 1, n_padded_samples)

		unfolded_input = unfold(padded_audio, original_batch_size, n_channel, self.chunk_size, self.hop_size)

		n_total_chunks, n_channel, chunk_size = unfolded_input.shape

		n_batch = np.ceil(n_total_chunks / self.batch_size).astype(int)

		chunks_in = [unfolded_input[b * self.batch_size : (b + 1) * self.batch_size, ...].clone() for b in range(n_batch)]

		all_chunks_out = defaultdict(lambda: torch.zeros_like(unfolded_input, device="cpu"))
		...
		for b, cin in enumerate(chunks_in):
			...
			for s, c in chunks_out.items():
				all_chunks_out[s][b * self.batch_size : (b + 1) * self.batch_size, ...] = c.cpu()
			del chunks_out
...
class LinearFader(BaseFader):
	def __init__(self, chunk_size_second: float, hop_size_second: float, fs: int, fade_edge_frames: bool = False, batch_size: int = 1) -> None:
		assert hop_size_second >= chunk_size_second / 2

		super().__init__(chunk_size_second=chunk_size_second, hop_size_second=hop_size_second, fs=fs, fade_edge_frames=fade_edge_frames, batch_size=batch_size)
		...
		self.register_buffer("standard_window", torch.concat([in_fade, center_ones, out_fade]))

		self.fade_edge_frames = fade_edge_frames
		self.edge_frame_pad_size = (self.overlap_size, self.overlap_size)

		if not self.fade_edge_frames:
			self.first_window = nn.Parameter(torch.concat([inout_ones, center_ones, out_fade]), requires_grad=False)
			self.last_window = nn.Parameter(torch.concat([in_fade, center_ones, inout_ones]), requires_grad=False)


class OverlapAddFader(BaseFader):
	def __init__(self, window_type: str, chunk_size_second: float, hop_size_second: float, fs: int, batch_size: int = 1) -> None:
		assert (chunk_size_second / hop_size_second) % 2 == 0
		assert int(chunk_size_second * fs) % 2 == 0

		super().__init__(chunk_size_second=chunk_size_second, hop_size_second=hop_size_second, fs=fs, fade_edge_frames=True, batch_size=batch_size)
...
if __name__ == "__main__":
	...
	audio_, _ = ta.load("$DATA_ROOT/MUSDB18/HQ/canonical/test/BKS - Too " "Much/vocals.wav")
	audio_ = audio_[None, ...]
