...
class SignalNoisePNormRatio(_Loss):
	def __init__(self, p: float = 1.0, scale_invariant: bool = False, zero_mean: bool = False, take_log: bool = True, reduction: str = "mean", EPS: float = 1e-3) -> None:
	...
	def forward(self, est_target: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
		...
		if self.scale_invariant:
			...
			s_target_energy = torch.sum(target * torch.conj(target), dim=-1, keepdim=True)
...
class MultichannelSingleSrcNegSDR(_Loss):
	def __init__(self, sdr_type: str, p: float = 2.0, zero_mean: bool = True, take_log: bool = True, reduction: str = "mean", EPS: float = 1e-8) -> None:
		assert reduction != "sum", NotImplementedError
		super().__init__(reduction=reduction)
	...
	def forward(self, est_target: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
		...
		if self.p == 2.0:
			losses = torch.sum(scaled_target**2, dim=[1, 2]) / (torch.sum(e_noise**2, dim=[1, 2]) + self.EPS)
		else:
			losses = torch.norm(scaled_target, p=self.p, dim=[1, 2]) / (torch.linalg.vector_norm(e_noise, p=self.p, dim=[1, 2]) + self.EPS)
		if self.take_log:
