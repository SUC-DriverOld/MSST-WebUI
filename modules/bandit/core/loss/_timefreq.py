...
class TimeFreqWrapper(_Loss):
	def __init__(self, time_module: _Loss, freq_module: Optional[_Loss] = None, time_weight: float = 1.0, freq_weight: float = 1.0, multistem: bool = True) -> None:
		super().__init__()
	...
	def forward(self, preds: Any, target: Any) -> torch.Tensor:
		return self.time_weight * self.time_module(preds, target) + self.freq_weight * self.freq_module(preds, target)


class TimeFreqL1Loss(TimeFreqWrapper):
	def __init__(self, time_weight: float = 1.0, freq_weight: float = 1.0, tkwargs: Optional[Dict[str, Any]] = None, fkwargs: Optional[Dict[str, Any]] = None, multistem: bool = True) -> None:
		if tkwargs is None:
			tkwargs = {}
		if fkwargs is None:
			fkwargs = {}
