...
def get_band_specs_map(band_specs_map, n_fft, fs, n_bands=None):
	if band_specs_map == "musdb:all":
		...
		bsm_, freq_weights, overlapping_band = get_band_specs("dnr:speech", n_fft, fs, n_bands)
...
class SingleMaskMultiSourceBandSplitBase(BandSplitWrapperBase, _SpectralComponent):
	def __init__(
		...
		super().__init__(
			n_fft=n_fft, win_length=win_length, hop_length=hop_length, window_fn=window_fn, wkwargs=wkwargs, power=power, center=center, normalized=normalized, pad_mode=pad_mode, onesided=onesided
		)

		if isinstance(band_specs_map, str):
			self.band_specs_map, self.freq_weights, self.overlapping_band = get_band_specs_map(band_specs_map, n_fft, fs, n_bands=n_bands)
