...
class ResidualRNN(nn.Module):
	def __init__(self, emb_dim: int, rnn_dim: int, bidirectional: bool = True, rnn_type: str = "LSTM", use_batch_trick: bool = True, use_layer_norm: bool = True) -> None:
		...
		self.rnn = rnn.__dict__[rnn_type](input_size=emb_dim, hidden_size=rnn_dim, num_layers=1, batch_first=True, bidirectional=bidirectional)
	...
	def forward(self, z):
		...
		if self.use_layer_norm:
			...
			z = torch.permute(z, (0, 3, 1, 2))  # (batch, emb_dim, n_uncrossed, n_across)

			z = self.norm(z)  # (batch, emb_dim, n_uncrossed, n_across)

			z = torch.permute(z, (0, 2, 3, 1))  # (batch, n_uncrossed, n_across, emb_dim)
...
class SeqBandModellingModule(TimeFrequencyModellingModule):
	def __init__(self, n_modules: int = 12, emb_dim: int = 128, rnn_dim: int = 256, bidirectional: bool = True, rnn_type: str = "LSTM", parallel_mode=False) -> None:
		super().__init__()
		...
		if parallel_mode:
			for _ in range(n_modules):
				self.seqband.append(
					nn.ModuleList(
						[
							ResidualRNN(emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, rnn_type=rnn_type),
							ResidualRNN(emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, rnn_type=rnn_type),
						]
					)
				)
			...
			for _ in range(2 * n_modules):
				self.seqband.append(ResidualRNN(emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, rnn_type=rnn_type))
...
class ResidualTransformer(nn.Module):
	def __init__(self, emb_dim: int = 128, rnn_dim: int = 256, bidirectional: bool = True, dropout: float = 0.0) -> None:
		# n_group is the size of the 2nd dim
		super().__init__()

		self.tf = nn.TransformerEncoderLayer(d_model=emb_dim, nhead=4, dim_feedforward=rnn_dim, batch_first=True)
...
class TransformerTimeFreqModule(TimeFrequencyModellingModule):
	def __init__(self, n_modules: int = 12, emb_dim: int = 128, rnn_dim: int = 256, bidirectional: bool = True, dropout: float = 0.0) -> None:
		super().__init__()
		self.norm = nn.LayerNorm(emb_dim)
		self.seqband = nn.ModuleList([])

		for _ in range(2 * n_modules):
			self.seqband.append(ResidualTransformer(emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, dropout=dropout))
...
class ResidualConvolution(nn.Module):
	def __init__(self, emb_dim: int = 128, rnn_dim: int = 256, bidirectional: bool = True, dropout: float = 0.0) -> None:
		...
		self.conv = nn.Sequential(nn.Conv2d(in_channels=emb_dim, out_channels=rnn_dim, kernel_size=(3, 3), padding="same", stride=(1, 1)), nn.Tanhshrink())

		self.is_causal = not bidirectional
		self.dropout = dropout

		self.fc = nn.Conv2d(in_channels=rnn_dim, out_channels=emb_dim, kernel_size=(1, 1), padding="same", stride=(1, 1))
...
class ConvolutionalTimeFreqModule(TimeFrequencyModellingModule):
	def __init__(self, n_modules: int = 12, emb_dim: int = 128, rnn_dim: int = 256, bidirectional: bool = True, dropout: float = 0.0) -> None:
		super().__init__()
		self.seqband = torch.jit.script(nn.Sequential(*[ResidualConvolution(emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, dropout=dropout) for _ in range(2 * n_modules)]))
