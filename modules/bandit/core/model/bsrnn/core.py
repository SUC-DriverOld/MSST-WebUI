...
from modules.bandit.core.model.bsrnn.maskestim import MaskEstimationModule, OverlappingMaskEstimationModule
from modules.bandit.core.model.bsrnn.tfmodel import ConvolutionalTimeFreqModule, SeqBandModellingModule, TransformerTimeFreqModule
...
class SingleMaskBandsplitCoreRNN(SingleMaskBandsplitCoreBase):
	def __init__(
		self,
		in_channel: int,
		band_specs: List[Tuple[float, float]],
...
class SingleMaskBandsplitCoreTransformer(SingleMaskBandsplitCoreBase):
	def __init__(
		self,
		in_channel: int,
		band_specs: List[Tuple[float, float]],
		...
		self.tf_model = TransformerTimeFreqModule(n_modules=n_sqm_modules, emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, dropout=tf_dropout)
		self.mask_estim = MaskEstimationModule(
			in_channel=in_channel,
			band_specs=band_specs,
			emb_dim=emb_dim,
...
class MultiSourceMultiMaskBandSplitCoreTransformer(MultiMaskBandSplitCoreBase):
	def __init__(
		self,
		in_channel: int,
		stems: List[str],
...
class MultiSourceMultiMaskBandSplitCoreConv(MultiMaskBandSplitCoreBase):
	def __init__(
		self,
		in_channel: int,
		stems: List[str],
		...
		self.tf_model = ConvolutionalTimeFreqModule(n_modules=n_sqm_modules, emb_dim=emb_dim, rnn_dim=rnn_dim, bidirectional=bidirectional, dropout=tf_dropout)
...
class PatchingMaskBandsplitCoreBase(MultiMaskBandSplitCoreBase):
	...
	def mask(self, x, m):
		...
		xf = F.unfold(x, kernel_size=(kernel_freq, kernel_time), padding=padding, stride=(1, 1))

		xf = xf.view(-1, n_channel, kernel_freq, kernel_time, n_freq, n_time)

		sf = xf * m

		sf = sf.view(-1, n_channel * kernel_freq * kernel_time, n_freq * n_time)

		s = F.fold(sf, output_size=(n_freq, n_time), kernel_size=(kernel_freq, kernel_time), padding=padding, stride=(1, 1)).view(-1, n_channel, n_freq, n_time)
	...
	def old_mask(self, x, m):
		...
		for ifreq in range(kernel_freq):
			for itime in range(kernel_time):
				...
				s[:, :, fslice, tslice] += x[:, :, fslice, tslice] * m[ifreq, itime, :, :, fslice, tslice]
