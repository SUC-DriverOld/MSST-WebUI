...
class NormFC(nn.Module):
	def __init__(self, emb_dim: int, bandwidth: int, in_channel: int, normalize_channel_independently: bool = False, treat_channel_as_feature: bool = True) -> None:
		super().__init__()
...
class BandSplitModule(nn.Module):
	def __init__(
		...
		self.norm_fc_modules = nn.ModuleList(
			[  # type: ignore
				(NormFC(emb_dim=emb_dim, bandwidth=bw, in_channel=in_channel, normalize_channel_independently=normalize_channel_independently, treat_channel_as_feature=treat_channel_as_feature))
