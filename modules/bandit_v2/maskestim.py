...
class BaseNormMLP(nn.Module):
	def __init__(self, emb_dim: int, mlp_dim: int, bandwidth: int, in_channels: Optional[int], hidden_activation: str = "Tanh", hidden_activation_kwargs=None, complex_mask: bool = True):
		super().__init__()
		if hidden_activation_kwargs is None:
			hidden_activation_kwargs = {}
		self.hidden_activation_kwargs = hidden_activation_kwargs
		self.norm = nn.LayerNorm(emb_dim)
		self.hidden = nn.Sequential(nn.Linear(in_features=emb_dim, out_features=mlp_dim), activation.__dict__[hidden_activation](**self.hidden_activation_kwargs))
...
class NormMLP(BaseNormMLP):
	def __init__(self, emb_dim: int, mlp_dim: int, bandwidth: int, in_channels: Optional[int], hidden_activation: str = "Tanh", hidden_activation_kwargs=None, complex_mask: bool = True) -> None:
		super().__init__(
			emb_dim=emb_dim,
			mlp_dim=mlp_dim,
			bandwidth=bandwidth,
		...
		self.output = nn.Sequential(nn.Linear(in_features=mlp_dim, out_features=bandwidth * in_channels * self.reim * 2), nn.GLU(dim=-1))

		try:
			self.combined = torch.compile(nn.Sequential(self.norm, self.hidden, self.output), disable=True)
...
class OverlappingMaskEstimationModule(MaskEstimationModuleBase):
	...
	def forward(self, q):
		...
		masks = torch.zeros((batch, self.in_channels, self.n_freq, n_time), device=q.device, dtype=torch.complex64)
