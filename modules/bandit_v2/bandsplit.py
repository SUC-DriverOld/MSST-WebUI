...
class NormFC(nn.Module):
	def __init__(self, emb_dim: int, bandwidth: int, in_channels: int, normalize_channel_independently: bool = False, treat_channel_as_feature: bool = True) -> None:
		super().__init__()
		...
		try:
			self.norm_fc_modules = nn.ModuleList(
				[  # type: ignore
					torch.compile(
						NormFC(
							emb_dim=emb_dim, bandwidth=bw, in_channels=in_channels, normalize_channel_independently=normalize_channel_independently, treat_channel_as_feature=treat_channel_as_feature
						),
						disable=True,
					)
			...
			self.norm_fc_modules = nn.ModuleList(
				[  # type: ignore
					NormFC(emb_dim=emb_dim, bandwidth=bw, in_channels=in_channels, normalize_channel_independently=normalize_channel_independently, treat_channel_as_feature=treat_channel_as_feature)
					for bw in self.band_widths
				]
			)
