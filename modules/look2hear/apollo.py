...
class Roformer(nn.Module):
	...
	def _calc_rotary_emb(self):
		freq = 1.0 / (self.theta ** (torch.arange(0, self.hidden_size, 2)[: (self.hidden_size // 2)] / self.hidden_size))  # theta_i
...
class Apollo(BaseModel):
	def __init__(self, sr: int, win: int, feature_dim: int, layer: int):
		...
		for i in range(self.nband):
			self.BN.append(nn.Sequential(RMSNorm(self.band_width[i] * 2 + 1), nn.Conv1d(self.band_width[i] * 2 + 1, self.feature_dim, 1)))
		...
		for i in range(self.nband):
			self.output.append(nn.Sequential(RMSNorm(self.feature_dim), nn.Conv1d(self.feature_dim, self.band_width[i] * 4, 1), nn.GLU(dim=1)))

	def spec_band_split(self, input):
		B, nch, nsample = input.shape

		spec = torch.stft(input.view(B * nch, nsample), n_fft=self.win, hop_length=self.stride, window=torch.hann_window(self.win).to(input.device), return_complex=True)
	...
	def forward(self, input):
		...
		output = torch.istft(est_spec, n_fft=self.win, hop_length=self.stride, window=torch.hann_window(self.win).to(input.device), length=nsample).view(B, nch, -1)
