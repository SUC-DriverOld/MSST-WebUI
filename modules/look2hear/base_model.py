...
def pad_to_appropriate_length(x, lcm):
	...
	if values_to_pad:
		...
		padded_x = torch.zeros(list(appropriate_shape[:-1]) + [appropriate_shape[-1] + lcm - values_to_pad], dtype=torch.float32).to(x.device)
...
class BaseModel(nn.Module):
	...
	def serialize(self):
		...
		# Additional infos
		infos = dict()
		infos["software_versions"] = dict(torch_version=torch.__version__, pytorch_lightning_version=pl.__version__)
