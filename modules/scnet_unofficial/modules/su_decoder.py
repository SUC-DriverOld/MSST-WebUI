...
from modules.scnet_unofficial.utils import get_convtranspose_output_padding
...
class FusionLayer(nn.Module):
	...
	def __init__(self, input_dim: int, kernel_size: int = 3, stride: int = 1, padding: int = 1):
		"""
		Initializes FusionLayer with input dimension, kernel size, stride, and padding.
		"""
		super().__init__()
		self.conv = nn.Conv2d(input_dim * 2, input_dim * 2, kernel_size=(kernel_size, 1), stride=(stride, 1), padding=(padding, 0))
...
class Upsample(nn.Module):
	...
	def __init__(self, input_dim: int, output_dim: int, stride: int, output_padding: int):
		...
		"""
		super().__init__()
		self.conv = nn.ConvTranspose2d(input_dim, output_dim, 1, (stride, 1), output_padding=(output_padding, 0))
...
class SULayer(nn.Module):
	...
	def __init__(self, input_dim: int, output_dim: int, upsample_stride: int, subband_shape: int, sd_interval: Tuple[int, int]):
		"""
		...
		upsample_output_padding = get_convtranspose_output_padding(input_shape=sd_shape, output_shape=subband_shape, stride=upsample_stride)
		self.upsample = Upsample(input_dim=input_dim, output_dim=output_dim, stride=upsample_stride, output_padding=upsample_output_padding)
...
class SUBlock(nn.Module):
	...
	def __init__(self, input_dim: int, output_dim: int, upsample_strides: List[int], subband_shapes: List[int], sd_intervals: List[Tuple[int, int]]):
		"""
		...
		self.su_layers = nn.ModuleList(
			SULayer(input_dim=input_dim, output_dim=output_dim, upsample_stride=uss, subband_shape=sbs, sd_interval=sdi)
			for i, (uss, sbs, sdi) in enumerate(zip(upsample_strides, subband_shapes, sd_intervals))
		)
