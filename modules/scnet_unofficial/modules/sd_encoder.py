...
class ConvolutionModule(nn.Module):
	...
	def __init__(self, input_dim: int, hidden_dim: int, kernel_sizes: List[int], bias: bool = False) -> None:
		"""
		...
		self.sequential = nn.Sequential(
			...
			nn.Conv1d(input_dim, 2 * hidden_dim, kernel_sizes[0], stride=1, padding=(kernel_sizes[0] - 1) // 2, bias=bias),
			nn.GLU(dim=1),
			nn.Conv1d(hidden_dim, hidden_dim, kernel_sizes[1], stride=1, padding=(kernel_sizes[1] - 1) // 2, groups=hidden_dim, bias=bias),
			nn.GroupNorm(num_groups=1, num_channels=hidden_dim),
			nn.SiLU(),
			nn.Conv1d(hidden_dim, input_dim, kernel_sizes[2], stride=1, padding=(kernel_sizes[2] - 1) // 2, bias=bias),
		)
...
class SDLayer(nn.Module):
	...
	def __init__(self, subband_interval: Tuple[float, float], input_dim: int, output_dim: int, downsample_stride: int, n_conv_modules: int, kernel_sizes: List[int], bias: bool = True):
		"""
		...
		conv_modules = [ConvolutionModule(input_dim=output_dim, hidden_dim=output_dim // 4, kernel_sizes=kernel_sizes, bias=bias) for _ in range(n_conv_modules)]
...
class SDBlock(nn.Module):
	...
	def __init__(self, input_dim: int, output_dim: int, bandsplit_ratios: List[float], downsample_strides: List[int], n_conv_modules: List[int], kernel_sizes: List[int] = None):
		"""
		...
		self.sd_layers = nn.ModuleList(
			SDLayer(input_dim=input_dim, output_dim=output_dim, subband_interval=sbi, downsample_stride=dss, n_conv_modules=ncm, kernel_sizes=kernel_sizes)
			for sbi, dss, ncm in zip(subband_intervals, downsample_strides, n_conv_modules)
		)
