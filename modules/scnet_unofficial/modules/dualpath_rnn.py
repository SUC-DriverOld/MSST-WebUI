...
class MambaModule(nn.Module):
	def __init__(self, d_model, d_state, d_conv, d_expand):
		...
		self.mamba = Mamba(d_model=d_model, d_state=d_state, d_conv=d_conv, d_expand=d_expand)
...
class DualPathRNN(nn.Module):
	...
	def __init__(self, n_layers: int, input_dim: int, hidden_dim: int, use_mamba: bool = False, d_state: int = 16, d_conv: int = 4, d_expand: int = 2):
		"""
		...
		for i in range(1, n_layers + 1):
			...
			layer = nn.ModuleList([net(**kwargs), net(**kwargs), RFFTModule(inverse=(i % 2 == 0))])
