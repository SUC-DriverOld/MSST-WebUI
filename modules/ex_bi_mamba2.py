...
class Mamba2(nn.Module):
	...
	def forward(self, u: Tensor):
		...
		xBC = silu(self.conv1d(xBC.transpose(1, 2)).transpose(1, 2)[:, : u.shape[1], :])  # (batch, seqlen, d_inner + 2 * d_state))
		...
		y = self.ssd(x * dt.unsqueeze(-1), A * dt, B.unsqueeze(2), C.unsqueeze(2))
...
class BiMamba2_2D(_BiMamba2):
	...
	def forward(self, x):
		h, w = x.shape[2:]
		x = F.pad(x, (0, (8 - x.shape[3] % 8) % 8, 0, (8 - x.shape[2] % 8) % 8))  # 将 h , w  pad到8的倍数, [b, c64, h8, w8]
		_b, _c, _h, _w = x.shape
...
class BiMamba2_3D(_BiMamba2):
	...
	def forward(self, x):
		d, h, w = x.shape[2:]
		x = F.pad(x, (0, (4 - x.shape[4] % 4) % 4, 0, (4 - x.shape[3] % 4) % 4, 0, (4 - x.shape[2] % 4) % 4))  # 将 d, h, w , pad到4的倍数, [b, c64,d4, h4, w4]
...
if __name__ == "__main__":
	# 通用的多维度双向mamba2
	from torchnssd import export_jit_script, export_onnx, statistics, test_run
