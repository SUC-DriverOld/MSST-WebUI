...
class MambaBlock(nn.Module):
	def __init__(self, in_channels):
		...
		self.forward_mamba2 = Mamba2(d_model=in_channels, d_state=128, d_conv=4, expand=4, headdim=64)

		self.backward_mamba2 = Mamba2(d_model=in_channels, d_state=128, d_conv=4, expand=4, headdim=64)
...
class TAC(nn.Module):
	...
	def __init__(self, input_size, hidden_size):
		...
		self.TAC_output = nn.Sequential(nn.Linear(hidden_size * 2, input_size), nn.Tanh())
...
class Separator(nn.Module):
	def __init__(self, sr=44100, win=2048, stride=512, feature_dim=128, num_repeat_mask=8, num_repeat_map=4, num_output=4):
		...
		for i in range(self.nband):
			self.BN_mask.append(nn.Sequential(nn.GroupNorm(1, self.band_width[i] * 2, self.eps), nn.Conv1d(self.band_width[i] * 2, self.feature_dim, 1)))

		self.BN_map = nn.ModuleList([])
		for i in range(self.nband):
			self.BN_map.append(nn.Sequential(nn.GroupNorm(1, self.band_width[i] * 2, self.eps), nn.Conv1d(self.band_width[i] * 2, self.feature_dim, 1)))
	...
	def forward(self, input):
		...
		spec = torch.stft(input, n_fft=self.win, hop_length=self.stride, window=torch.hann_window(self.win).to(input.device).type(input.type()), return_complex=True)
