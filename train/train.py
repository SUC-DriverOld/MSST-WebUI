...
from train.valid import valid_multi_gpu, valid
...
def train_model(args):
	...
	parser.add_argument(
		"--metrics", nargs="+", type=str, default=["sdr"], choices=["sdr", "l1_freq", "si_sdr", "log_wmse", "aura_stft", "aura_mrstft", "bleedless", "fullness"], help="List of metrics to use."
	)
	parser.add_argument(
		"--metric_for_scheduler", default="sdr", choices=["sdr", "l1_freq", "si_sdr", "log_wmse", "aura_stft", "aura_mrstft", "bleedless", "fullness"], help="Metric which will be used for scheduler."
	)
	if args is None:
	...
	logger.info("Metrics for training: {}. Metric for scheduler: {}".format(args.metrics, args.metric_for_scheduler))
	...
	trainset = MSSDataset(config, args.data_path, batch_size=batch_size, metadata_path=os.path.join(args.results_path, "metadata_{}.pkl".format(args.dataset_type)), dataset_type=args.dataset_type)
	...
	logger.info(
		"Patience: {} Reduce factor: {} Batch size: {} Grad accum steps: {} Effective batch size: {} Optimizer: {}".format(
			config.training.patience, config.training.reduce_factor, batch_size, gradient_accumulation_steps, batch_size * gradient_accumulation_steps, config.training.optimizer
		)
	)
			...
			with torch.cuda.amp.autocast(enabled=use_amp):
				if args.model_type in ["mel_band_roformer", "bs_roformer"]:
					...
					if args.use_multistft_loss:
						...
						loss = masked_loss(y_, y, q=config.training.q, coarse=config.training.coarse_loss_clip)
