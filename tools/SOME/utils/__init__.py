...
def random_continuous_masks(*shape: int, dim: int, device: str | torch.device = "cpu"):
	start, end = torch.sort(
		torch.randint(low=0, high=shape[dim] + 1, size=(*shape[:dim], 2, *((1,) * (len(shape) - dim - 1))), device=device).expand(*((-1,) * (dim + 1)), *shape[dim + 1 :]), dim=dim
	)[0].split(1, dim=dim)
	idx = torch.arange(0, shape[dim], dtype=torch.long, device=device).reshape(*((1,) * dim), shape[dim], *((1,) * (len(shape) - dim - 1)))
...
def batch_by_size(indices, num_frames_fn, max_batch_frames=80000, max_batch_size=48, required_batch_size_multiple=1):
	...
	for i in range(len(indices)):
		...
		assert sample_len <= max_batch_frames, "sentence at index {} of size {} exceeds max_batch_samples " "limit of {}!".format(idx, sample_len, max_batch_frames)
...
def load_ckpt(cur_model, ckpt_base_dir, ckpt_steps=None, prefix_in_ckpt="model", key_in_ckpt="state_dict", strict=True, device="cpu"):
	...
	if ckpt_base_dir.is_file():
		...
		checkpoint_path = sorted(
			[ckpt_file for ckpt_file in base_dir.iterdir() if ckpt_file.is_file() and re.fullmatch(r"model_ckpt_steps_\d+\.ckpt", ckpt_file.name)],
	...
	if prefix_in_ckpt is not None:
		state_dict = OrderedDict({k[len(prefix_in_ckpt) + 1 :]: v for k, v in state_dict.items() if k.startswith(f"{prefix_in_ckpt}.")})
	if not strict:
...
def build_lr_scheduler_from_config(optimizer, scheduler_args):
	...
	def helper(params):
		if isinstance(params, list):
			...
			if "cls" in resolved:
				if resolved["cls"] == "torch.optim.lr_scheduler.ChainedScheduler" and scheduler_args["scheduler_cls"] == "torch.optim.lr_scheduler.SequentialLR":
...
def simulate_lr_scheduler(optimizer_args, scheduler_args, step_count, num_param_groups=1):
	optimizer = build_object_from_class_name(
		optimizer_args["optimizer_cls"], torch.optim.Optimizer, [{"params": torch.nn.Parameter(), "initial_lr": optimizer_args["lr"]} for _ in range(num_param_groups)], **optimizer_args
	)
