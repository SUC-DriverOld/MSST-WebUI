...
from webui.utils import i18n, load_configs, save_configs, load_vr_model, get_vr_model, load_msst_model, get_msst_model, logger, detailed_error
	...
	def vr_infer(self, model_name, input_folder, output_dir, output_format="wav"):
		...
		vr_inference = multiprocessing.Process(
			...
			args=(
				...
				self.aggression,
				self.use_tta,
				self.enable_post_process,
				self.post_process_threshold,
...
def preset_inference(input_folder, store_dir, preset_name, force_cpu, output_format, use_tta, extra_output_dir: bool, is_audio=False):
	...
	logger.info(f"\033[33mPreset: {preset_name} inference process completed, results saved to {store_dir}, " f"time cost: {round(time.time() - start_time, 2)}s\033[0m")
