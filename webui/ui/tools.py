...
def tools(webui_config):
        ...
        with gr.TabItem(label=i18n("音频格式转换")):
            ...
            with gr.Row():
                sample_rate = gr.Radio(
                    ...
                    value=webui_config['tools']['sample_rate'] if webui_config['tools']['sample_rate'] else 44100,
                    interactive=True
                )
                channels = gr.Radio(
                    label=i18n("输出音频声道数"),
            ...
            with gr.Row():
                ...
                flac_bit_depth = gr.Radio(
                    ...
                    value=webui_config['tools']['flac_bit_depth'] if webui_config['tools']['flac_bit_depth'] else "16-bit",
                    interactive=True
                )
    ...
    merge_audio_button.click(
        fn=merge_audios,
        inputs=[
            merge_audio_input,
            merge_audio_output
        ],
        outputs=output_message_merge
    )
